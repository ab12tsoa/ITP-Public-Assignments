/*
DiatomRecognizer by Allison Berman based on
...
Images come from the microscope videos online
*/
...
SVM model;
int[] labels; // 1 = cymbella, 2 = gyrosigma, 3 = stauroneis, 4 = Nitzchia, 5 = other diatom
...
//----------------
  ...
  for (int i = 1; i < trainingFilenames.length; i++) {
//    println(trainingFilenames[i]);
    // split the filename by "_" and look at the first part
    // to decide the label
    String gestureLabel = split(trainingFilenames[i], '_')[0];
    if (gestureLabel.equals("cymbella")) {
      labels[i] = 1;
    }

    if (gestureLabel.equals("gyrosigma")) {
      labels[i] = 2;
    }

    if (gestureLabel.equals("stauroneis")) {
      labels[i] = 3;
    }
//
//    if (gestureLabel.equals("Nitzchia")) {
//      labels[i] = 4;
//    }

//    if (gestureLabel.equals("pinnularia")) {
//      labels[i] = 5;
//    }
//
//    if (gestureLabel.equals("other")) {
//      labels[i] = 6;
//    }
  ...
  model.saveModel("diatom_model.txt");
...
double testNewImage() {
  ...
  int imgNum = (int)random(1, testFilenames.length-1);
  // load a test image
...
void draw() {
  ...
  case 1:
    fill(255, 125, 125);
    result = result + "cymbella";
    break;
  case 2:
    fill(125, 255, 125);
    result = result + "gyrosigma";
    break;
  case 3:
    fill(125, 125, 255);
    result = result + "stauroneis";
    break;
//  case 4:
//    fill(125, 255, 255);
//    result = result + "Nitzchia";
//    break;
//  case 5:
//    fill(255, 255, 125);
//    result = result + "pinnularia";
//    break;
//  case 6:
//    fill(255);
//    result = result + "other";
